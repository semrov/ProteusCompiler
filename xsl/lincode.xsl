		...
		<tr>
			<td align="center" style="background-color:#E69D26">
				<table cellspacing="0">
    				...
    				<tr>
    					<xsl:apply-templates select="iminstruction"/>
    				</tr>
...
<xsl:template match="iminstruction">
    	<td align="center" valign="top">
			<table cellspacing="0">
				<tr>
					<td align="center" style="background-color:#4D97CA">
						<xsl:variable name="instr" select="@instr"/>
						<xsl:variable name="value" select="@value"/>
						<xsl:value-of select="$instr"/>
						<nobr><xsl:if test="$value">(<xsl:value-of select="$value"/>)</xsl:if></nobr>
					</td>
				</tr>
				<tr>
					<td align="center">
						<table cellspacing="0">
							<tr>
								<xsl:for-each select="iminstruction">
									<td valign="top">
										<xsl:apply-templates select="."/>
									</td>
								</xsl:for-each>
